...
export function RegisterForm() {
  ...
  const notifyRegisterSuccess = (name: string) =>
    toast.success(`You're Registered ${name}!`);
  ...
  const handleUserRegistration = async (userData: RegisterUserData) => {
    try {
      NProgress.start();
      console.log( ' registration          going on');

      const result = await registerUser(userData);
      console.log(result + ' registration          susses successfully');
    ...
    <Card className=" p-6 sm:p-10 lg:p-12">
      ...
      <Form {...form}>
        ...
        <form
          ...
          className="space-y-5 min-w-64 max-w-lg"
        >
          ...
          <Button
            type="submit"
            className="w-full hover:bg-primaryBlue  bg-primaryBlue"
          >
            Submit
          </Button>
        </form>
