...
function Hero() {
  ...
  const handleFetchGeoData = async () => {
    ...
    const city = geoData.results[0].address_components.find((component: any) =>
  ...
  const handleSearch = async () => {
    ...
    router.push(
      `/property?address=${encodeURIComponent(
        location
      )}&lat=${lat}&lng=${lng}&gender=all`
    );
  };
    ...
    <div className="w-10/12 lg:w-5/6 max-w-6xl gap-12 flex flex-row justify-between items-center h-96 ">
      <div className="flex flex-col w-96 align-left justify-left text-left gap-8">
        <div className="flex flex-col gap-2 sm:gap-6 ">
          {/* <p className='text-4xl font-extrabold text-start sm:text-5xl'>Find the <span className='text-primaryBlue'>Right Match, Share the Right Space</span>The Smart Way to Find a Roommate</p> */}
          <p className="text-4xl font-extrabold text-start sm:text-5xl">
            Find the <span className="text-primaryBlue">Right Match</span>,{" "}
            <br />
            <span className="text-primaryBlue">The Smart Way</span> to Find a
            Roommate
          </p>
          ...
          <TypeAnimation
            sequence={[
              ...
              "Connecting You to Your Ideal Roommate",
              1000, // wait 1s before replacing "Mice" with "Hamsters"
              "Matching You with the Right Roommate Vibes!",
              1000,
              '"Find the Right Match, Share the Right Space"!',
              1000,
            ]}
            wrapper="p"
            cursor={true}
        ...
        <div className="flex flex-col gap-4">
          ...
          <p className="text-slate-500">
            ...
            <Link href="/city/Pune" className="hover:underline cursor-pointer">
              Pune.
            </Link>
          </p>
        </div>
      </div>
      <div className="hidden lg:flex h-full w-5/6 relative ">
        {/* Wrapping the Image in motion.div for continuous motion */}
        <motion.div
          initial={{ opacity: 1, x: -50 }} // Start animation with opacity 0 and slightly off-screen
          animate={{ opacity: 1, x: 0 }} // End animation at full opacity and the correct position
          transition={{
            duration: 3, // Set animation duration
            repeat: Infinity, // Repeat infinitely
            repeatType: "reverse", // Reverse the direction after every cycle
            ease: "easeInOut", // Add smooth easing for better motion
          }}
