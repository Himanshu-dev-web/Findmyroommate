...
function PersonalDetails() {
  ...
  const [isCountrySuggestionSelected, setIsCountrySuggestionSelected] =
    useState<boolean>(false);
  const [isCountrySuggestionsLoading, setIsCountrySuggestionsLoading] =
    useState<boolean>(false);
  ...
  const fetchCountryPredictions = async (search: string) => {
    ...
    try {
      ...
      // Extract the common names from the response data
      const suggestions = data?.map((country: any) => {
        return {
          name: country.name.common, // Extract the common name
          countryCode: country.cca2, // You can also include the country code if needed
        };
      });

      //console.log(`Suggestions: ${JSON.stringify(suggestions)}`);

      // Set the country suggestions in the state
  ...
  const [isCitySuggestionSelected, setIsCitySuggestionSelected] =
    useState<boolean>(false);
  const [isCitySuggestionsLoading, setIsCitySuggestionsLoading] =
    useState<boolean>(false);
    ...
    try {
      ...
      if (selectedCountryCode == null) {
        response = await fetch(
          `http://api.geonames.org/searchJSON?q=${search}&maxRows=10&featureClass=P&username=jolones`
        );
      } else {
        response = await fetch(
          `http://api.geonames.org/searchJSON?q=${search}&country=${selectedCountryCode}&maxRows=10&featureClass=P&username=jolones`
        );
      }
      const data = await response.json();
    ...
    <div className="w-3/4 lg:w-1/2">
      ...
      <Card className=" p-6 sm:p-10 lg:p-12 ">
        ...
        <div className="flex flex-col gap-6 ">
            ...
            <div className="w-full flex gap-4 lg:w-52">
              <Button
                ...
                className={`w-full lg:w-24 ${
                  gender === "Male" ? "bg-primaryBlue hover:bg-primaryBlue" : ""
                }`}
          ...
          <div className="flex flex-col gap-4 lg:flex-row justify-between items-center">
            <p className="text-left w-full lg:w-min font-semibold city-row">
              City
            </p>
            <div className="w-full relative flex gap-4 lg:w-52">
              <Command className="border border-input !border-b-0">
            ...
            <div className="w-full lg:w-52">
                ...
                <div className="min-w-52">
                  <div className="relative w-20 h-20">
                    <Image
                      ...
                      className="absolute rounded-full border-black bg-white right-2 cursor-pointer"
                      width={18}
                      height={18}
                      alt="avatar/>"
                    />
              ...
              {isModalOpen && (
                <AvatarCropPopup
                  ...
                  onCroppedImageFile={setCroppedImageFile}
                />
              )}
            </div>
          </div>
            ...
            <div className="flex w-full flex-wrap gap-2 mt-6 justify-center">
              {preferences.map((preference, index) => {
                return (
                  <button
                    key={index}
                    className={`h-8 flex hover:transform hover:scale-105 transition-transform duration-100 ease-in-out justify-center items-center px-6 border-2 rounded-xl border-slate-200 ${
          ...
          <Button
            className="w-full sm:w-1/2 left-0 right-0 m-auto mt-6 bg-primaryBlue hover:bg-primaryBlue"
