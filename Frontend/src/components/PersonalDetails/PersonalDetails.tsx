...
function PersonalDetails() {
  ...
  const fetchCountryPredictions = async (search: string) => {
    ...
    try {
      const response = await fetch(`http://api.geonames.org/searchJSON?q=${search}&maxRows=10&featureClass=A&featureCode=PCLI&username=hims213174`)
  ...
  console.log('Submit')
    if (gender == null) {
      ...
      try {
        ...
        NProgress.start();
        console.log(gender,country,city,croppedImageFile,selectedPreferences);
